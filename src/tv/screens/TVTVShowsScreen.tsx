...
interface TVTVShowsScreenProps {
  navigation: any;
}

function TVShowCard({ item, onItemPress, themeColor }: { item: any, onItemPress: any, themeColor: string }) {
  ...
  const displayTitle = item.name || item.title || 'Untitled';
...
export default function TVTVShowsScreen({ navigation }: TVTVShowsScreenProps) {
  ...
  const title = t('tv.tv_series') || 'TV Series';
  const subtitle = t('tv.tv_series_subtitle') || 'Discover popular and trending television series.';

  const fetchTVShows = async (pageNum: number, isInitial = false) => {
    if (isInitial) setLoading(true);
    else setLoadingMore(true);

    try {
      const resp: any = await tmdbApi.discoverTV(pageNum);
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TVShowCard
              item={item}
              onItemPress={(movieItem: any) => navigation.navigate('TVDetail', { item: movieItem, isTV: true })}
        ...
        <View style={styles.emptyContainer}>
          ...
          <View style={styles.loaderBox}>
            ...
            <Text style={styles.noRoomsText}>{t('tv.no_tv_shows') || 'No TV shows found.'}</Text>
          </View>
        </View>
      )}
    </View>
