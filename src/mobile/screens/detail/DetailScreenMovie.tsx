    ...
    const checkTracks = async () => {
      ...
      try {
        const dbResult = await authApi.getRecentlyWatchedItem(
          item.id.toString(), selectedServer, selectedAudio, isTV
        ).catch(() => null);
      ...
      if (!apiLinksFetched.current) {
        try {
          const [s1Tracks, s3Links] = await Promise.all([
            phimApi.getStreamingLink(item.id.toString(), title, parseInt(year), 1, isTV, 1).catch(() => null),
            nguoncApi.getStreamingLink(isTV, title, parseInt(year), director, 1, 1).catch(() => null)
          ]);
