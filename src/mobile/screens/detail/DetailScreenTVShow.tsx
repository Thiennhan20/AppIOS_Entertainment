...
export default function DetailScreenTVShow({ route, navigation }: any) {
  ...
  // Reset states when item.id changes
  useEffect(() => {
  ...
  const country = details?.production_countries?.[0]?.name || details?.production_countries?.[0]?.iso_3166_1 || details?.origin_country?.[0] || '';
  const director = details?.credits?.crew?.find((c:any) => c.job === 'Director')?.name || '';

  // Check available streams: DB first, then API if DB has no saved watchUrl
  useEffect(() => {
    ...
    const checkTracks = async () => {
      ...
      try {
        const dbResult = await authApi.getRecentlyWatchedItem(
          item.id.toString(), selectedServer, selectedAudio, isTV, initSeason, initEpisode
        ).catch(() => null);
      ...
      if (!apiLinksFetched.current) {
        try {
          const [s1Tracks, s3Links] = await Promise.all([
            phimApi.getStreamingLink(item.id.toString(), title, parseInt(year), initEpisode, isTV, initSeason).catch(() => null),
            nguoncApi.getStreamingLink(isTV, title, parseInt(year), director, initSeason, initEpisode).catch(() => null)
          ]);
  ...
  }, [title, year, selectedServer, selectedAudio]);

  const activeAudioList = selectedServer === 'Server 1' ? s1Available : s3Available;

  const handlePlay = async () => {
    if (checkingStreams && !savedWatchUrl) return;
    
    try {
      // Priority: use saved watchUrl if available
      if (savedWatchUrl) {
        navigation.navigate('PlayerScreen', { 
          item, isTV, 
          streamUrl: savedWatchUrl, 
          activePlayer: savedWatchUrl.includes('.m3u8') ? 'm3u8' : 'embed',
      ...
      >
        ...
        <View style={styles.contentContainer}>
          ...
          <View style={styles.metaRow}>
            ...
            <Text style={styles.metaText}>{details?.episode_run_time?.[0] ? `${details.episode_run_time[0]}m` : (details?.number_of_seasons ? `${details.number_of_seasons} ${t('general.season', { defaultValue: 'Season' })}` : `1 ${t('general.season', { defaultValue: 'Season' })}`)}</Text>
          </View>
